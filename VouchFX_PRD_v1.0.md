...
| Program | Link | Reward | New user gets | Cap |
|---|---|---|---|---|
| Affiliate (cash) | `/r/CODE` | 20% recurring **cash** | 20% off first month | **12 months per referred user** (then 0%) |
| Referral (credit) | `/ref/CODE` | 20% recurring **account credit** | 20% off first month | applied credit never exceeds 100% of the referrer's own bill (excess rolls) |

| ID | Requirement | Priority | Acceptance criteria |
|---|---|---|---|
| VCH-REF-01 | Provider affiliate program: unique affiliate link `/r/CODE` + code; **20% recurring CASH commission** on each referred user's collected subscription payments, **capped at 12 months per referred user** (each referred user has their own 12-month window from their conversion date; after 12 months that user pays 0% commission) | M | An affiliate gets a working `/r/` link; a signup via it binds to them; commission accrues at 20% of each collected payment for 12 months from that user's conversion, then stops; the clock is per-referred-user, stored on the referral record (not the affiliate) |
| VCH-REF-02 | Affiliate dashboard: clicks, signups, trial→paid conversions, active referrals (with each user's months-remaining), earnings (pending/locked/paid), payout history | M | All figures display and update as referrals convert; each referred user shows its 12-month countdown |
| VCH-REF-03 | **Unified one-slot attribution**: a single attribution record per user across BOTH programs. Capture via cookie with a **60-day window**; bind and **lock** at signup. Precedence: an explicitly entered code at signup beats a cookie; otherwise last-touch within the window wins. Once locked it never changes. Exactly one source earns per subscriber | M | A user touched by both an affiliate and a referral link earns ONLY one source (never both); the attribution is one record, locked at signup, unchanged by later clicks |
| VCH-REF-04 | **Maturation gate + clawback**: commission/credit for a referred user becomes payable only after their first payment clears AND the 14-day refund window passes; accrues only on collected paid subscriptions (not trials); refund/chargeback claws back | M | Trials earn $0; a referred user's earnings are withheld until first payment clears + refund window passes; a refunded payment reverses its commission/credit |
| VCH-REF-05 | Affiliate payouts (cash): minimum threshold ($50), monthly cycle, via Paystack (NGN) / bank / Wise / crypto; method set by affiliate. Requesting a payout **locks** the balance (pending→processing), never zeroes it; clears only on paid, restores on failed (see VCH-ADMIN-03); disbursement actioned via the admin console (manual at launch) | M | Below threshold rolls over; at/above is payable; a requested-but-unpaid payout never destroys the balance |
| VCH-REF-06 | User referral program: every user has a referral link `/ref/CODE`; **referrer earns 20% recurring as account credit** while both stay subscribed; **referred user gets 20% off their first month**. Applied credit never exceeds 100% of the referrer's own bill; excess rolls over and never converts to cash | S | Referrer's invoice is credited (capped at their own bill, remainder rolls); referred user's first charge is discounted 20% |
| VCH-REF-07 | Share assets: one-tap copy of the relevant link (affiliate or referral), a ready-made Telegram message, and a QR code | S | User can copy link/message and download/scan QR for their link type |
| VCH-REF-08 | Fraud & abuse controls (concentrated on the CASH side): affiliate status is opt-in/approved (payout details required); self-referral blocked (email/device/payment fingerprint); reciprocal-loop detection (A→B→A); signup velocity checks per IP/device; manual payout review in the admin console. The one-slot rule already removes the largest gaming vector | M | Self-referral, reciprocal loops, and obvious duplicates earn no commission; affiliates are approved before earning cash |
| VCH-REF-09 | Co-branded landing page per top affiliate (channel name + their `/r/` link) | C | An affiliate can be issued a co-branded acquisition page |

**Commission model (finalised, closes R9):** Affiliate = 20% recurring **cash**, capped at **12 months per referred user** (per-user clock from conversion), then 0%. Referral = 20% recurring **account credit**, capped so applied credit never exceeds the referrer's own bill (excess rolls, never cashable). One earner per subscriber via unified one-slot attribution (VCH-REF-03). Maturation gate: payable only after first payment clears + 14-day refund window (VCH-REF-04). Lifetime purchases pay affiliate 20% one-off (no recurring). This caps LTV tail liability, keeps the credit program self-limiting, and makes double-payout structurally impossible — see §14 R9 (resolved).
...
| R9 | Referral/affiliate commission — **RESOLVED** (§6.11): affiliate 20% **cash** capped at **12 months per referred user** (per-user clock); referral 20% **account credit** capped at the referrer's own bill; one earner per subscriber via unified one-slot attribution; maturation gate (first payment clears + 14-day refund window) before payable; Lifetime pays 20% one-off. Caps the LTV tail, makes the credit program self-limiting, and makes double-payout structurally impossible | Client | Resolved |
