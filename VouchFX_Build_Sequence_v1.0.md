...
## Prompt P1.20b — PWA shell (installable + push)
```
Add a LIGHT PWA to apps/web (VCH-PWA-01..04, PRD §6.13) — installability + web push only, NO offline/data caching. (1) Web app manifest with VouchFX icons (maskable set), dark theme color, standalone display, so the app is installable on Android/iOS/desktop. (2) A minimal service worker that handles install + push events only — it must NOT cache API responses, routes, or trades data. (3) Web Push (VAPID): request notification permission IN-CONTEXT (e.g. after the user enables an alert, not on first load), store push subscriptions per device in Postgres, and send pushes for the same VCH-NOT-01 events (trade opened/closed, broker disconnect, daily-loss-cap hit, Telegram session limited) — reusing the existing per-event notification toggles so push is a third channel alongside in-app + email, not a separate system. (4) Subscription management: list/revoke push per device; detect iOS and guide the user to "Add to Home Screen" first (iOS only delivers push to installed PWAs). Add VAPID keys to the env schema. Do not build offline mode.
```
