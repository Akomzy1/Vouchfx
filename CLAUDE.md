...
1. **Never double-place a trade.** Every execution is idempotent. A trade may be opened for a signal only if no `trades` row already exists for that `parsed_signal_id` **on the same `broker_connection_id`** in an OPEN/PENDING state. (Multi-account copying, VCH-BRK-04: the same signal legitimately opens one trade PER copy-enabled account, so the guard — and every follow-up/cancel/close matcher — is scoped by `(parsed_signal_id, broker_connection_id)`.) See the trade-execution skill.
2. **Idempotency key = `(telegram_chat_id, telegram_message_id, edit_version)`**, extended per account to `(…, broker_connection_id)` for the BullMQ job id so a fan-out to N accounts is N distinct jobs (not deduped) while staying idempotent within each account. This identifies a signal-per-account uniquely across retries, worker restarts, and duplicate deliveries.
